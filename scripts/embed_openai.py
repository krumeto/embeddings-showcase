"""Generate OpenAI embeddings for a prepared parquet dataset."""
...
from embeddings_showcase.embeddings import embed_with_openai


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--input", default="data/raw/newswire_5000_sample.parquet")
    parser.add_argument("--output", default="data/embeddings/openai_embeddings.npy")
...
def main() -> None:
    ...
    for index, batch in enumerate(batches(texts, args.batch_size), start=1):
        vectors = embed_with_openai(batch, model=args.model)
        parts.append(vectors)
        print(f"Embedded OpenAI batch {index:,} ({len(batch):,} rows)", flush=True)
